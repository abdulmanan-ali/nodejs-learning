...
// Model
const userModel = mongoose.model("user", userSchema); // first parameter 'user' is model name and automatically changed it's in plural
...
app
  ...
  .patch(async (req, res) => {
    ...
    return res.status(201).json({ status: "user updated" });
  })
